...
$stmt = $pdo->prepare("
    ...
    JOIN administradores op ON op.id = p.id_operador
    WHERE p.id = ?
");
$stmt->execute([$id]);
$pago = $stmt->fetch();
...
<head>
<meta charset="UTF-8">
<title>Recibo <?= h($pago['correlativo']) ?> | Portal de Notas</title>
...
<style>
  ...
  .recibo-correlativo { font-size: 15px; font-weight: 700; color: var(--azul); }
      ...
      <div class="recibo">
        ...
        <div class="recibo-cabecera">
          <div>
            ...
            <div style="font-size:12.5px; color:var(--texto-secundario);"><?= h($pago['sede']) ?></div>
          </div>
          <div style="text-align:right;">
            <div class="recibo-correlativo"><?= h($pago['correlativo']) ?></div>
            <div style="font-size:12.5px; color:var(--texto-secundario);"><?= h(date('d/m/Y H:i:s', strtotime($pago['creado_en']))) ?> (<?= date_default_timezone_get() ?>)</div>
          </div>
        </div>
        ...
        <div class="recibo-grid">
          ...
          <div><span class="rotulo">Concepto</span><?= h($pago['concepto']) ?></div>
        ...
        <div class="recibo-total">Total pagado: $<?= number_format((float)$pago['monto_total'], 2) ?></div>
